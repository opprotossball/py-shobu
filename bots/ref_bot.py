...
class RefBot:
    ...
    def __init__(s):
        ...
        s.minimax_calls = 0
        s.tt_hits = 0
        s.transposition_table = HashTable(size_limit=RefBot.TRANSPOSITION_TABLE_SIZE)

    def choose_move(s, position):
        s.minimax_calls = 0
        s.tt_hits = 0
        position_hash = s.zobrist.get_hash(position)
        eval, move_id = s.negamax(position, position_hash, RefBot.MAX_DEPTH, -RefBot.INF, RefBot.INF, 1 if position.white_to_go else -1)
        print(f'tt hits: {s.tt_hits}')
        return position.get_legal_moves()[move_id]

    # def guess_move_strength(s, position, move):
    #     pst = RefBot.WHITE_PST if position.white_to_go else RefBot.BLACK_PST
    ...
    def eval(s, position):
        ...
        for board in position.all_boards():
            for i in position.board_iterator():
                if board[i] == Shobu.WHITE_PIECE:
                    eval += RefBot.PIECE_VALUE
                    eval += RefBot.WHITE_PST[position.internal_2_readable(i)]
                elif board[i] == Shobu.BLACK_PIECE:
                    eval -= RefBot.PIECE_VALUE
                    eval -= RefBot.BLACK_PST[position.internal_2_readable(i)]
        # eval = 0
        # for board in position.all_boards():
        #     for i in position.board_iterator():
        ...
        #             material_advantage -= 1
        #             eval -= RefBot.BLACK_PST[position.internal_2_readable(i)]
        #         eval += s.sign(material_advantage) *  RefBot.SQUARED_MATERIAL_ADVANTAGE_VALUE * (material_advantage ** 2)
                
        # mobility = len(position.get_legal_moves()) * RefBot.MOBILITY_WEIGHT
